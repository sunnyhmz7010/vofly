...
- 卡资料、余额历史、计划按 (iccid, profile_aid) 隔离；实体 SIM 的 profile_aid 为空串；存储查找不回退设备 ID。
- 查看非激活 Profile 不切卡；立即查询与余额自动查询按现有自动任务逻辑切卡，完成后目标 Profile 保持激活并恢复其原有网络策略。
- 续费/保号提醒不执行 USSD/SMS、不切卡；只有 balance_query 计划真正执行余额查询。
- 通知发到所有已配置且启用的渠道，不新增渠道选择字段。
- 自定义链接仅 HTTP(S)；不新增第三方依赖；文件 UTF-8 + LF；提交信息中文。

**Task 3 后端 Profile 感知余额查询 + 可复用执行边界（未做）**

- 改 internal/server/{balance_api.go,automatic_tasks.go}、internal/balance/{service.go,types.go,store_adapter.go}；新建 internal/server/profile_execution.go(+_test)、query_center_balance_test.go。
- 把 automatic_tasks.go 的 ensureAutomaticTaskProfile / prepareAutomaticTaskEnvironment / restoreAutomaticTaskEnvironment（:229/:278/:516）行为抽成 server 私有执行边界（入参设备 ID、目标 Profile ICCID/AID、操作回调；返回活跃物理设备并按现有语义恢复目标卡网络状态），现有自动任务改调它且语义不变。
- profile_aid/change 字段 Task 1 已铺进 internal/balance 与 internal/store；本任务补齐 DeviceSnapshot、入站匹配与 JSON 输出缺口，然后挂路由：POST /query-center/balance-queries（device_id、iccid、profile_aid，仅目标 Profile 非激活才切卡，调 balances.StartQuery，不走命令中心执行 API）、GET /query-center/balance-queries?iccid=…&profile_aid=…；分发加在 query_center_api.go 的 routeQueryCenterAPI（general_api.go:36 已挂载）。
- 验证 `go test ./internal/balance ./internal/server -count=1`（含既有自动任务回归）；提交信息「支持查询中心按 Profile 查询余额」。

**Task 4 后端余额计划调度器与通知（未做；store 层 balance_plans 表 Task 1 已建）**

- 新建 internal/server/balance_plans.go(+_test)、balance_plan_notifications.go；改 query_center_api.go、automatic_task_notifications.go（启用渠道分发抽成共享 helper）、cmd/vofly/main.go。
...
- StartBalancePlans(ctx)：每 server 一个调度器，原子 claim 到期计划；事务内先推进 next_run_at 再派发（重启不重复领取）；按设备串行执行；持久化 last_run_at/last_status/last_error（成败都记）；启动点放 cmd/vofly/main.go:297 的 StartAutomaticTasks 旁。
- 执行：balance_query → 共享执行边界 + 现有余额服务；renewal_reminder → 仅构造卡级提醒（卡/Profile 标签、配置间隔、计划时间），不切卡不查询。notify 为真时遍历启用渠道发送，单渠道失败仅记日志；不改 TG 命令注册与自动任务通知语义；补离线设备、缺 Profile、查询超时、通知失败、服务重启、360 天提醒等用例。
- 验证 `go test ./internal/store ./internal/balance ./internal/server -count=1` 与 `go vet`；提交信息「增加余额自动查询和卡级提醒」。

**Task 5 前端数据契约（本仓库，未做）**

- 改 src/types.ts；新建 src/lib/queryCenter.ts、test/queryCenter.test.mjs。
- 类型用 camelCase（api.ts 自动做 camel/snake 转换）：CardResource、KnowledgeLink、BalancePlan，BalanceQuery 增 profileAid、previousAmount、changeAmount、changeDirection。
...
- SMS 风格桌面三栏 + 移动端下钻；实体 SIM 一个上下文、eSIM Profile 按 EID 分组；查看 Profile 只加载数据绝不切卡；设备/iccid/profile_aid/menu 同步进 URL search 参数；保留 /commands → /query-center 兼容跳转。
- 余额面板：当前解析余额、最近变化、查询状态、历史；「立即查询」POST 所选设备/Profile、pending 防重复点击、刷新历史、透出离线/缺规则/pending/切卡失败错误；附自动查询计划表单紧凑入口。
- 卡资料面板：可编辑充值/续费 URL、有序知识链接多行、保存/恢复默认（后端 GET 已返回 effective/defaults/customized，恢复后即时反映默认值）、校验反馈、外链按钮；不落 localStorage。
...
- 后端 `go test ./...` + `go vet ./...`，确认既有自动任务与消息渠道命令测试不变且通过；前端 `npm test` + `npm run build`。
- Playwright 浏览器 smoke（桌面 + 移动布局）：设备 → Profile → 菜单导航、非激活 Profile 查询载荷、恢复默认、多条知识链接、360 天提醒表单、空/错/载入态；截图仅调试用，不部署、不重启远程 VM。
